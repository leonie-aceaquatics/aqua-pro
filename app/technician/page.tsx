...
export default function TechnicianPage() {
  ...
  const [showAccess, setShowAccess] = useState(false)        // Site access: keys, codes, parking for every site
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', maxWidth: '480px', margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '16px 20px', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <button onClick={() => setShowAccess(true)} title="Site access — keys and codes" style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', display: 'flex' }}>
              <KeyRound size={19} />
            </button>
      ...
      {selected && !showTestForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', zIndex: 200 }}
          ...
          <div style={{ background: 'var(--surface)', borderTop: '1px solid var(--border)', borderRadius: '20px 20px 0 0', padding: '24px', width: '100%', maxWidth: '480px', margin: '0 auto' }}>
            ...
            {selected.pools?.access_notes && (
              <div style={{ background: '#fdcb6e18', border: '1px solid #fdcb6e50', borderRadius: '10px', padding: '12px 14px', marginBottom: '16px' }}>
                <div style={{ fontSize: '11px', color: '#fdcb6e', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>
      ...
      {/* Site access — every site's keys, codes and parking notes */}
      {showAccess && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', zIndex: 400, overflowY: 'auto' }}>
          <div style={{ padding: '20px', maxWidth: '480px', margin: '0 auto', paddingBottom: '40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
              <button onClick={() => setShowAccess(false)} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: '8px', color: '#e2e8f0', padding: '8px 14px', cursor: 'pointer' }}>
                ← Back
              </button>
              <div style={{ fontWeight: '700', fontSize: '16px', color: '#e2e8f0' }}><KeyRound size={16} style={{ verticalAlign: '-3px', marginRight: '6px' }} />Site access</div>
            </div>
            <div style={{ color: '#64748b', fontSize: '13px', marginBottom: '16px' }}>Keys, codes, parking and where to find the plant room. Don&apos;t screenshot or share this outside the crew.</div>
            {[...allPools].sort((a, b) => (a.name ?? '').localeCompare(b.name ?? '')).filter(p => p.access_notes).map(p => (
              <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px 16px', marginBottom: '10px' }}>
            ...
            {allPools.filter(p => p.access_notes).length === 0 && (
              <div style={{ color: '#64748b', fontSize: '13px', textAlign: 'center', padding: '32px 0' }}>No access notes entered yet — the office adds them under Pools.</div>
            )}
          </div>
        </div>
      )}
